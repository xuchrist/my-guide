<template>
   <yd-layout>
     ...
     <v-header slot="navbar" title="游客列表" to="/admin_home" > 
    </v-header>
...
export default {
  ...
  methods: {
     goDetail:function(item){ 
      this.$router.push({path:'/admin_visitor_info',query:{obj:item}});//类似get传参，通过URL传递参数
    // this.$router.push({path:'/visitor_cash_info',params:{obj:item}});//类似post传参 
    }
  },
  created () {
      this.list= [
