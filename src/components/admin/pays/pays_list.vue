<template>
      <yd-layout>
    <v-header slot="navbar" title="充值列表" to="/admin_home">
    </v-header>
      <yd-list theme=4>
...
export default {
  ...
  data () {
    ...
    goDetail:function(item){ 
      this.$router.push({path:'/admin_news_info',query:{obj:item}});//类似get传参，通过URL传递参数
    // this.$router.push({path:'/admin_cash_info',params:{obj:item}});//类似post传参 
    }
  },
  created () {
     this.list= [
